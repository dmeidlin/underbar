...
(function () {
  ...
  _.reduce = function (collection, iterator, accumulator) {
    ...
    var workingCollection = collection;
    if (accumulator === undefined) {
      result = workingCollection[0];
      workingCollection.shift();
    }
  ...
  _.every = function (collection, test) {
    ...
    return _.reduce(collection, function (accumulator, element) {
      return accumulator && !!test(element);
    }, true);
  };
  // _.every = function (collection, iterator) {
  //   return _.contains(_.map(collection, iterator), false) === false;
  ...
  _.some = function (collection, test) {
    ...
    return !_.every(collection, falseTest);
  ...
  _.memoize = function (func) {
    ...
    var memoInner = function () {
      var args = [];
      var i;
      for (i = 0; i < arguments.length; i++) {
        args[i] = arguments[i];
      }
      if (_.contains(passedValues, args[0])) {
        result = returnedValues[_.indexOf(passedValues, args[0])];
      } else {
        result = func.apply(this, arguments);
        passedValues.push(args[0]);
